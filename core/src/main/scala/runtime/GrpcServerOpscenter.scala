...
import freestyle.opscenter.protocols.OpscenterService
import freestyle.rpc.server._
import journal.Logger
import monix.eval.Task
import routeguide.handlers.OpscenterServiceHandler

import scala.concurrent.{Await, ExecutionContext, Future}
import scala.concurrent.duration._

trait GrpcServerOpscenterEC {
...
trait GrpcServerOpscenter extends GrpcServerOpscenterEC with FutureInstances with TaskInstances

object server {

  trait Implicits extends GrpcServerOpscenter with GrpcServerConf {

    import freestyle.rpc.server.handlers._
    import freestyle.rpc.server.implicits._

    implicit val routeGuideServiceHandler: OpscenterService.Handler[Future] =
      new OpscenterServiceHandler[Future]

    val grpcConfigs: List[GrpcConfig] = List(
      AddService(OpscenterService.bindService[OpscenterService.Op, Future])
    )
